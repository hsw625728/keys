...
- (id) init {
	if((self=[super init])) {
		...
		for(NSUInteger i=0; i<achievements.numAchievements; i++) {
			...
			// text
			CCLabelTTF* achievment_label_top = [CCLabelTTF labelWithString:topText fontName:@"RTWSYueGoTrial-Regular.otf" fontSize:24];
			achievment_label_top.position = ccp(194+320*page, 107+68*y);
			[pageLayer addChild:achievment_label_top z:3];
			CCLabelTTF* achievment_label_bottom = [CCLabelTTF labelWithString:bottomText fontName:@"RTWSYueGoTrial-Regular.otf" fontSize:18];
